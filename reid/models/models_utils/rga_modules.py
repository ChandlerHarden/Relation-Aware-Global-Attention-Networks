...
class RGA_Module(nn.Module):
	...
	def forward(self, x):
		...
		if self.use_spatial:
			# spatial attention					
			# torch.Size([8, 256, 64, 32])
			theta_xs = self.theta_spatial(x)
			# torch.Size([8, 32, 64, 32]) nong suo cheng 32zhang tu de feature
			...
			if not self.use_channel:
				...
				x = F.sigmoid(W_ys.expand_as(x)) * x # x * weight
			# x does not change shape 
		...
		if self.use_channel:
			...
			Gc_in = Gc.permute(0, 2, 1).unsqueeze(-1) # 8 256 256
			Gc_out = Gc.unsqueeze(-1) # 8 256 256 1 diao huan
			...
			W_yc = self.W_channel(yc).transpose(1, 2) # 8 256 1 1
			out = F.sigmoid(W_yc) * x # 8 256 64 32
